...
##transform JD_CODE columns to numeric
cols = c(2:ncol(data_jd))
data_jd[,cols] = apply(data_jd[,cols], 2, function(x) as.numeric(as.character(x)))
...
data_jd_as_datatable <- data.table::data.table(data_jd)
...
data_for_model <- merge(data0_as_datatable, data_jd_as_datatable, by = "RUID")
...
s_model_jd_features <- paste(colnames(data_jd_as_datatable)[2:length(colnames(data_jd_as_datatable))], collapse="+")
...
all_features <- paste(c(s_model_data0_features, s_model_jd_features) , collapse="+")

## this model_SYSTOLIC_0
