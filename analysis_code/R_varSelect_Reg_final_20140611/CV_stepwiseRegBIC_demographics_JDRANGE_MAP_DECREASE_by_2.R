...
for (i in 1:num_fold) {
	...
	num_training_examples = dim(training_set)[1]
	...
	model_MAP_DECREASE_2_allfeatures_train <- step(model_MAP_DECREASE_2_0_train, scope=list(lower=~1, upper=f), direction="forward", k=log10(num_training_examples))  	
...
#replace "1" that gets added to end of a factor variable
l_stepwise_features_used_unique = gsub(".991",".99", l_stepwise_features_used_unique)
l_stepwise_features_used_unique = gsub("S1","S", l_stepwise_features_used_unique)
l_stepwise_features_used_unique = gsub("B1","B", l_stepwise_features_used_unique)
l_stepwise_features_used_unique = gsub("N1","N", l_stepwise_features_used_unique)
l_stepwise_features_used_unique = gsub("E1","E", l_stepwise_features_used_unique)



#find all features used by #times out of 10 folds
...
df_CV_features_used_byFreq =  as.data.frame(sort(table(l_allfeatures_allfolds_in_regression)))
print(df_CV_features_used_byFreq)
l_features_for_cluster = row.names(subset(df_CV_features_used_byFreq, sort(table(l_allfeatures_allfolds_in_regression))>=8))
l_features_for_cluster = gsub(".991",".99", l_features_for_cluster)
l_features_for_cluster = gsub("S1","S", l_features_for_cluster)
l_features_for_cluster = gsub("B1","B", l_features_for_cluster)
l_features_for_cluster = gsub("N1","N", l_features_for_cluster)
l_features_for_cluster = gsub("E1","E", l_features_for_cluster)
...
workspace_filename <- paste(c("CV_stepwiseRegBIC_cluster_demographics_JDRANGE_MAP_DECREASE_by_2", curr_date_time, ".RData"), collapse="_")
