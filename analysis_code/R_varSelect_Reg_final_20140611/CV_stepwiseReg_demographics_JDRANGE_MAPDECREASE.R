...
l_auc_all_folds = c()
num_fold = 10
cases = subset(data_for_model_scaled, MAP_DECREASE==1)
controls = subset(data_for_model_scaled, MAP_DECREASE==0)
num_cases = dim(cases)[1]
num_controls = dim(controls)[1]
l_stepwise_features_used_per_fold = list()

for (i in 1:num_fold) {
	rm(model_MAP_DECREASE_0_train)
	rm(model_MAP_DECREASE_allfeatures_train)
	...
	model_MAP_DECREASE_0_train <- glm(MAP_DECREASE~1, training_set , family = binomial)
	#TRAINING SET: formulas for each feature set
	formula <- paste(c("MAP_DECREASE", s_model_features_to_use), collapse="~")
	#TRAINING SET: formula for all features
	(f <- as.formula(paste("~", s_model_features_to_use)))
	#TRAINING SET: stepwise ADD
	model_MAP_DECREASE_allfeatures_train <- step(model_MAP_DECREASE_0_train, scope=list(lower=~1, upper=f), direction="forward")  	

	#TESTING SET - fit the training set model to the TESTING set
	predictions_testing_set <- predict(model_MAP_DECREASE_allfeatures_train, newdata=testing_set, type="response")
	testing_set$predictions <- predictions_testing_set
	roc_this_fold <- roc(MAP_DECREASE ~ predictions, data = testing_set)
	auc_this_fold <- roc_this_fold$auc
	l_auc_all_folds <- append(l_auc_all_folds, auc_this_fold)

	##list of features used this fold
	l_features_this_fold <- names(coefficients(model_MAP_DECREASE_allfeatures_train))[-1]
