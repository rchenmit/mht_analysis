...
for (i in 1:num_fold) {
	rm(model_SYSTOLIC_OR_DIASTOLIC_DECREASE_2_0_train)
	rm(model_SYSTOLIC_OR_DIASTOLIC_DECREASE_2_allfeatures_train)
	...
	formula <- paste(c("SYSTOLIC_OR_DIASTOLIC_DECREASE_2", s_model_features_to_use), collapse="~")
...
## plot ROC and AUC
#plot.new()
#plot.roc(data_for_model_scaled$SYSTOLIC_OR_DIASTOLIC_DECREASE_2, fitted(model_SYSTOLIC_OR_DIASTOLIC_DECREASE_2_allfeatures), print.auc=TRUE,
#													#arguments for CI
...
workspace_filename <- paste(c("stepwiseReg_demographics_JDRANGE_SYSTOLIC_OR_DIASTOLIC_DECREASE_2", curr_date_time, ".RData"), collapse="_")
save.image(workspace_filename)

## Run cluster
source('cluster_SYSTOLIC_OR_DIASTOLIC_DECREASE_2.R')
