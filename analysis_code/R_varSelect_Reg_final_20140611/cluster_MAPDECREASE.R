...
model_cluster <- Mclust(cluster_features, G=1:30, modelNames=mclust.options("emModelNames"))
summary(model_cluster)
names(model_cluster)

model_predicted = as.data.frame(model_cluster$classification)
actual_targets = as.data.frame(data_for_model_scaled$MAP_DECREASE)
...
write.csv(df_mean_sum_features_by_cluster[, names_cluster_features], file="cluster_mean_sum_MAPDECREASE.csv")

save.image("cluster_MAPDECREASE.RData")
